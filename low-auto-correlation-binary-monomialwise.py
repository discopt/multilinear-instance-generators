...
def generate(N, R, outFilePrefix, factor=1):
  ...
  outFile = open(outFilePrefix + '.lp', 'w')
  ...
  for term in sortedTerms:
    ...
    sumSingles = ' + '.join(f'x_{i}' for i in term)
    outFile.write(f'large_{termVar}: - {termVar} + {sumSingles} <= {len(term) - 1}\n')
    for i in term:
      outFile.write(f'small_{termVar}_x_{i}: {termVar} - x_{i} <= 0\n')
  outFile.write('bounds\n')
  for i in range(1,N+1):
    outFile.write(f'  x_{i} <= 1\n')
  outFile.write('binary\n')
  outFile.write(' ' + ' '.join(f' x_{i}' for i in range(1,N+1)))
